...
def make_chart(col_name, ytitle, hover_suffix, height=340):
    ...
    fig.update_layout(height=height, plot_bgcolor="white", hovermode="x unified", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    return fig
...
for em, tit, col, ytit, suf in ACORDEONES:
    with st.expander(f"{em} {tit}", expanded=False):
...
if conf.get("api_v2_capable"):
    ...
    if st.button("📊 Generar Gráficos Detallados"):
        with st.spinner("Limpiando datos fantasma..."):
            ...
            try:
                ...
                if sensor_obj and "data" in sensor_obj:
                    ...
                    # --- FILTRO MAESTRO: CORTAR FÍSICAMENTE EL DATAFRAME ---
                    df_v2 = df_v2.sort_values('dt')
                    
                    if fecha_analisis == ahora_peru.date():
                        # Eliminamos todo lo que sea superior a la hora actual de Ica
                        limite = ahora_peru
                        df_v2 = df_v2[df_v2['dt'] <= limite].copy()

                    df_v2['H'] = df_v2['dt'].dt.strftime('%H:00')
                    
                    # Nombres de columnas según sensor
                    ...
                    # Rango X forzado
                    rango_x = ["00:00", df_h['H'].iloc[-1]] if not df_h.empty else ["00:00", "23:00"]
                    ...
                    fig_t.update_layout(title=f"Curva Térmica: {fundo_sel}", xaxis_title="Hora", plot_bgcolor="white", height=500, hovermode="x unified", hoverlabel=dict(bgcolor="#1a1a1a", font_color="white"), legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
