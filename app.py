...
def obtener_datos_v1():
    try:
        ...
        # Usamos hoy_peru() para que coincida en local y nube
...
def cargar_todo(fundo):
    try:
        ...
        # Excluir fila de hoy en el CSV y reemplazar con la de la API
...
# Rango del gráfico histórico también basado en hora Perú
...
if conf.get("api_v2_capable"):
    ...
    with col_z1:
        # ✅ FIX: valor por defecto basado en hora Perú, no en hora del SO
        fecha_analisis = st.date_input("Seleccione el día:", hoy_peru())

    if st.button("📊 Generar Gráficos Detallados"):
        with st.spinner("Procesando datos horarios de Davis..."):

            # ✅ FIX: hora Perú calculada desde UTC — igual en local y en la nube
            now_peru = ahora_peru()
            hoy_p    = hoy_peru()
            ...
            try:
                ...
                if target_sensor and "data" in target_sensor:
                    ...
                    # Convertir timestamps a hora Perú (UTC-5)
                    df_v2['dt'] = pd.to_datetime(df_v2['ts'], unit='s') - timedelta(hours=5)

                    # ✅ FIX PRINCIPAL: si el día consultado es hoy en Perú,
                    # cortar los datos al momento actual real en Perú.
                    # Usamos >= para evitar fallos por microsegundos de desfase.
                    ...
                    fig_t.update_layout(
                        title=f"Curva Térmica Horaria: {fundo_sel} ({fecha_analisis})",
                    ...
                    fig_v.update_layout(
                        title=f"Viento y Humedad Horaria: {fundo_sel} ({fecha_analisis})",
