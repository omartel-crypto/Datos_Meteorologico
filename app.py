...
ahora_ref = datetime.now(tz_peru).replace(tzinfo=None)
fecha_fin_hist = ahora_ref
fecha_inicio_hist = ahora_ref - timedelta(days=31)
...
if conf.get("api_v2_capable"):
    ...
    if st.button("📊 Generar Gráficos Detallados"):
        with st.spinner("Limpiando datos y conectando con Davis..."):
            ...
            try:
                ...
                if sensor_obj and "data" in sensor_obj:
                    ...
                    # Convertimos a la hora de Perú
                    df_v2['dt'] = pd.to_datetime(df_v2['ts'], unit='s').dt.tz_localize('UTC').dt.tz_convert(tz_peru)
                    
                    # --- FILTRO AGRESIVO DE SEGURIDAD ---
                    # Si el día consultado es HOY, borramos físicamente cualquier fila mayor a la hora actual
                    ...
                    # Si es hoy, limitamos el eje X para que no queden huecos
                    if fecha_analisis == ahora_peru.date():
                        rango_x = ["00:00", ahora_peru.strftime('%H:00')]
                    else:
                        rango_x = ["00:00", "23:00"]
                    ...
                    fig_t.update_layout(title=f"Curva Térmica Horaria: {fundo_sel}", xaxis_title="Hora", plot_bgcolor="white", height=500, hovermode="x unified", hoverlabel=dict(bgcolor="#1a1a1a", font_size=13, font_color="white"), legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
                    ...
                    fig_v.update_layout(title=f"Viento y Humedad: {fundo_sel}", xaxis_title="Hora", plot_bgcolor="white", height=500, hovermode="x unified", hoverlabel=dict(bgcolor="#1a1a1a", font_size=13, font_color="white"), legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
